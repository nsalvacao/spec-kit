...
class TestExtensionManifest:
    ...
    def test_command_file_path_traversal_rejected(self, temp_dir, valid_manifest_data):
        """Test manifest rejects command files that escape extension directory."""
        import yaml

        valid_manifest_data["provides"]["commands"][0]["file"] = "../outside.md"
    ...
    def test_command_alias_path_traversal_rejected(self, temp_dir, valid_manifest_data):
        """Test manifest rejects unsafe alias values."""
        import yaml

        valid_manifest_data["provides"]["commands"][0]["aliases"] = ["../../evil"]

        manifest_path = temp_dir / "extension.yml"
        with open(manifest_path, 'w') as f:
            yaml.dump(valid_manifest_data, f)

        with pytest.raises(ValidationError, match="command alias"):
            ExtensionManifest(manifest_path)
...
class TestExtensionManager:
    ...
    def test_local_config_backup_on_remove(self, extension_dir, project_dir):
        """Test that local-config.yml is backed up on removal."""
        manager = ExtensionManager(project_dir)

        manager.install_from_directory(extension_dir, "0.1.0", register_commands=False)

        ext_dir = project_dir / ".specify" / "extensions" / "test-ext"
        local_config = ext_dir / "local-config.yml"
        local_config.write_text("token: local")

        manager.remove("test-ext", keep_config=False)

        backup_dir = project_dir / ".specify" / "extensions" / ".backup" / "test-ext"
        backup_file = backup_dir / "local-config.yml"
        assert backup_file.exists()
        assert backup_file.read_text() == "token: local"
...
"""
    ...
    def test_register_rejects_unsafe_source_path(self, project_dir, temp_dir):
        """Runtime safeguard: reject command source paths escaping extension dir."""
        ext_dir = temp_dir / "ext-runtime-path"
        ext_dir.mkdir()
        (ext_dir / "commands").mkdir()
        (ext_dir / "commands" / "cmd.md").write_text("---\ndescription: Test\n---\n\nTest")
        ...
        class FakeManifest:
            id = "test-ext"
            commands = [{"name": "speckit.test.safe", "file": "../outside.md"}]

        registrar = CommandRegistrar()
        with pytest.raises(ValidationError, match="path traversal"):
            registrar.register_commands_for_claude(FakeManifest(), ext_dir, project_dir)

    def test_register_rejects_unsafe_alias(self, project_dir, temp_dir):
        """Runtime safeguard: reject aliases with path traversal characters."""
        ext_dir = temp_dir / "ext-runtime-alias"
        ext_dir.mkdir()
        (ext_dir / "commands").mkdir()
        (ext_dir / "commands" / "cmd.md").write_text("---\ndescription: Test\n---\n\nTest")
...
class TestExtensionCatalog:
    ...
    def test_get_extension_info_invalid_extension_id(self, temp_dir):
        ...
        catalog = ExtensionCatalog(project_dir)
        with pytest.raises(ValidationError, match="Invalid extension ID"):
            catalog.get_extension_info("../evil")

    def test_download_extension_invalid_extension_id(self, temp_dir):
        """Test invalid extension IDs are rejected before download."""
        project_dir = temp_dir / "project"
        project_dir.mkdir()
        (project_dir / ".specify").mkdir()
        ...
        catalog = ExtensionCatalog(project_dir)
        with pytest.raises(ValidationError, match="Invalid extension ID"):
            catalog.download_extension("../evil")
