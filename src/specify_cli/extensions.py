...
def _validate_extension_id(extension_id: str, *, context: str = "extension ID") -> str:
    """Validate extension identifiers used in manifests/catalog operations."""
    if not isinstance(extension_id, str) or not re.match(r'^[a-z0-9-]+$', extension_id):
        raise ValidationError(
            f"Invalid {context} '{extension_id}': must be lowercase alphanumeric with hyphens only"
        )
    return extension_id


def _validate_command_identifier(identifier: str, *, context: str) -> str:
    """Validate command names/aliases used as output filenames."""
    if not isinstance(identifier, str) or not identifier:
        raise ValidationError(f"Invalid {context}: must be a non-empty string")
    if "/" in identifier or "\\" in identifier or ".." in identifier:
        raise ValidationError(f"Invalid {context} '{identifier}': path traversal is not allowed")
    if not re.match(r'^[a-z0-9][a-z0-9.-]*$', identifier):
        raise ValidationError(
            f"Invalid {context} '{identifier}': use lowercase alphanumeric, dots, and hyphens only"
        )
    return identifier


def _resolve_within(base_dir: Path, relative_path: str, *, context: str) -> Path:
    """Resolve a relative path and ensure it stays within base_dir."""
    candidate = Path(relative_path)
    if candidate.is_absolute():
        raise ValidationError(f"Invalid {context} '{relative_path}': absolute paths are not allowed")
    resolved = (base_dir / candidate).resolve()
    try:
        resolved.relative_to(base_dir.resolve())
    except ValueError:
        raise ValidationError(f"Invalid {context} '{relative_path}': path traversal is not allowed")
    return resolved
...
class ExtensionManifest:
    ...
    def _validate(self):
        ...
        for cmd in provides["commands"]:
            ...
            # Validate command file path remains extension-local
            cmd_file = cmd["file"]
            if not isinstance(cmd_file, str) or not cmd_file:
                raise ValidationError("Command file path must be a non-empty string")
            _resolve_within(self.path.parent, cmd_file, context="command file path")

            aliases = cmd.get("aliases", [])
            if aliases is None:
                aliases = []
            if not isinstance(aliases, list):
                raise ValidationError("Command aliases must be a list")
            for alias in aliases:
                _validate_command_identifier(alias, context="command alias")
        ...
        if keep_config:
            ...
            if extension_dir.exists():
                for child in extension_dir.iterdir():
                    # Keep top-level *-config.yml (including local-config.yml) and
                    # legacy *-config.local.yml files for backwards compatibility.
            ...
            if extension_dir.exists():
                ...
                # Backup both current and legacy local override naming conventions.
                config_files = set(extension_dir.glob("*-config.yml"))
                config_files.update(extension_dir.glob("*-config.local.yml"))
...
class CommandRegistrar:
    ...
    def register_commands_for_agent(
        ...
        for cmd_info in manifest.commands:
            cmd_name = _validate_command_identifier(cmd_info["name"], context="command name")
            cmd_file = cmd_info["file"]

            # Read source command file
            source_file = _resolve_within(extension_dir, cmd_file, context="command file path")
            ...
            for alias in cmd_info.get("aliases", []):
                safe_alias = _validate_command_identifier(alias, context="command alias")
                alias_file = commands_dir / f"{safe_alias}{agent_config['extension']}"
...
class ExtensionCatalog:
    ...
    DEFAULT_CATALOG_URL = "https://raw.githubusercontent.com/nsalvacao/spec-kit/main/extensions/catalog.json"
    ...
    def get_extension_info(self, extension_id: str) -> Optional[Dict[str, Any]]:
        ...
        Returns:
            Extension metadata or None if not found
        """
        _validate_extension_id(extension_id, context="extension ID")
        catalog = self.fetch_catalog()
        extensions = catalog.get("extensions", {})
    ...
    def download_extension(self, extension_id: str, target_dir: Optional[Path] = None) -> Path:
        ...
        extension_id = _validate_extension_id(extension_id, context="extension ID")
        ...
        version = re.sub(r'[^A-Za-z0-9._-]', '_', str(ext_info.get("version", "unknown")))
        zip_filename = f"{extension_id}-{version}.zip"
        zip_path = (target_dir / zip_filename).resolve()
        try:
            zip_path.relative_to(target_dir.resolve())
        except ValueError:
            raise ExtensionError(f"Invalid target path for extension download: {zip_filename}")
