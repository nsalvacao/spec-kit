...
AGENT_CONFIG = {
    ...
    "agy": {
        "name": "Antigravity",
        "folder": ".agent/",  # Root folder; workflows/rules are managed by scripts/templates
        "install_url": None,  # IDE-based
        "requires_cli": False,
    },
    "bob": {
...
def ensure_gitignore_security(project_path: Path, tracker: StepTracker | None = None) -> str:
    ...
    security_patterns = [
        "# Agent credential files (auto-generated by specify-cli)",
...
def ensure_project_config_from_template(project_path: Path, tracker: StepTracker | None = None) -> None:
    ...
    if project_config.exists():
        if tracker:
            tracker.add("project-config", "Project config setup")
            tracker.skip("project-config", "existing file preserved")
        return

    if not template_config.exists():
        if tracker:
            tracker.add("project-config", "Project config setup")
            tracker.skip("project-config", "template not found")
        return
    ...
    try:
        ...
        if tracker:
            tracker.add("project-config", "Project config setup")
            tracker.complete("project-config", "copied from template")
        else:
            console.print("[cyan]Initialized project config from template[/cyan]")
    except Exception as e:
        if tracker:
            tracker.add("project-config", "Project config setup")
            tracker.error("project-config", str(e))
        else:
            console.print(f"[yellow]Warning: Could not initialize project config: {e}[/yellow]")
...
def init(
    ...
    for key, label in [
        ...
        ("project-config", "Project config setup"),
        ("cleanup", "Cleanup"),
    ...
    with Live(tracker.render(), console=console, refresh_per_second=8, transient=True) as live:
        ...
        try:
            ...
            ensure_project_config_from_template(project_path, tracker=tracker)
    ...
    if agent_config:
        ...
        if gitignore_status == "added":
            security_message = (
                ...
                f"  • [cyan].specify/spec-kit.local.yml[/cyan]\n\n"
    ...
    steps_lines.append("   • [cyan]/speckit.validate[/] - Run Gate G0 validation")
    steps_lines.append("")
    ...
    steps_lines.append("   • [cyan]/speckit.clarify[/] - Clarify and de-risk spec ambiguities")
    ...
    steps_lines.append("   • [cyan]/speckit.amend[/] - Amend spec post-implementation")
    steps_lines.append("")
    steps_lines.append("   [bold cyan]Optional Quality Helpers[/bold cyan]")
    steps_lines.append("   • [cyan]/speckit.analyze[/] - Check cross-artifact alignment")
    steps_lines.append("   • [cyan]/speckit.checklist[/] - Validate requirements completeness")
    steps_lines.append("   • [cyan]/speckit.taskstoissues[/] - Convert tasks to GitHub issues")
...
def scope_detect(
    ...
    try:
        ...
        if input_json is not None:
            ...
            input_data = scope_detection_input_from_mapping(
                {
